...
/**
 * Línea de la crónica: las fases aparecen al hacer scroll y la fase centrada en pantalla
 * se convierte en la activa (actualiza el panel de Mutis). También se puede activar con clic/teclado.
 */
export function StoryTimeline({ phases, activeIndex, onActivate }: Props) {
  ...
  return (
    <ol className="relative space-y-4" aria-label="Fases de la crónica">
      <div className="absolute bottom-6 left-[1.35rem] top-6 w-px bg-line" aria-hidden />
      <motion.div
        className="absolute left-[1.35rem] top-6 w-px origin-top bg-gradient-to-b from-accent via-fuchsia-400 to-fuchsia-500"
          ...
          >
            <span
              className={cn(
                ...
                isActive
                  ? 'scale-110 border-accent bg-accent/15 text-accent-light shadow-[0_0_40px_-8px_rgb(234_179_8/0.5)]'
                  : isPast
                    ? 'border-fuchsia-400/60 bg-fuchsia-500/10 text-fuchsia-300'
            ...
            <button
              ...
              className={cn(
                'hud-corners-gold w-full rounded-2xl border p-5 text-left transition-all duration-500 sm:p-6',
                isActive
                  ? 'border-accent/40 bg-accent/[0.06] shadow-[0_0_40px_-8px_rgb(234_179_8/0.45)]'
            ...
            >
              ...
              <p className={cn('mt-4 font-mono text-[10.5px] uppercase tracking-[0.16em]', isActive ? 'text-fuchsia-300' : 'text-muted/70')}>
                MUTIS · {phase.mutis.status.replace('MUTIS · ', '')}
              </p>
            </button>
          </motion.li>
        )
