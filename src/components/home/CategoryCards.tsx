            ...
            return (
              <FadeIn key={category.slug} delay={index * 90}>
                <Link
                  id={category.slug}
                  to={category.href}
                  className={`group flex h-full scroll-mt-24 flex-col gap-4 rounded-2xl border border-navy/8 bg-white p-6 shadow-[0_2px_10px_-4px_rgba(9,38,74,0.1)] transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_18px_30px_-14px_rgba(9,38,74,0.25)] ${accent.border}`}
                >
                  <div className={`grid h-14 w-14 place-items-center rounded-full text-white ${accent.badge}`}>
